...
uint256 hashGenesisBlock("0x000006ddc9f3f533f28690c5391586ab991b8c4562cebb338d51c1c7990c8e69");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HunCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HunCoin Signed Message:\n";
...
{
    // HunCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // HunCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "HunCoin Genesis Block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1459027086;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 355126;

        if (fTestNet)
        {
            block.nTime    = 1459027085;
            block.nNonce   = 5261364;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xd74f6d7ae9ffa44f03713f0ea93e312f9a89f46c7d9701ad9c622f0dd49c9fe7"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0xcc, 0xb9, 0xda }; // HunCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HunCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HunCoinMiner : ProcessBlock, block not accepted");
    }
